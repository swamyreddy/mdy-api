...
exports.getProperties = (req, res, next) => {
    const priceRange = req.body.filter?.searchQuery?.priceRange || "";
    ...
    let propertyType = req.body.filter?.searchQuery?.propertyType;
    if (propertyType == 0) {
        propertyType = null;
    }
    let orderClause = [];
    let sort = req.body.filter?.searchQuery?.sort;
    if (sort === "low_to_high") {
    ...
    const searchValue = req.body.filter?.searchQuery?.location || "";
